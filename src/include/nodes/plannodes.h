...
typedef struct Scan {
    ...
    /* used for cypher query */
    char mm_type;                        /* 't': normal   table (default)
                                        * 'g': graph    table
                                        * 'd': document table
                                        * 'v': vector   table
                                        * 'a': array    table
                                        */
    CypherMatchExpr* cypher;
} Scan;
 ...
 */
// 数组扫描节点
// 从 "SELECT array_name[10:11][2:3][4:5](pm10)" 这样的查询入手，设计ArrayScan结构体
typedef struct ArrayScan {
    //内部封装的Scan
    Scan scan;
    ...
    //维度相关信息
    int dims_num;               //维度数量
    int* lower_bounds;          //每个维度的下界
    int* upper_bounds;           //每个维度的上界
    bool* lower_bounds_valid;    //每个维度的下界是否使用(为false时，表示维度下界未设置，即无下界)
    bool* upper_bounds_valid;    //每个维度的上界是否使用(为false时，表示维度上界未设置，即无上界)
    bool is_point_query;        //是否为点查询?(当所有维度的的上界和下界都相等时，即为点查询。点查询返回1个值而非数组)

    //属性相关信息
    int attrs_num;           //属性数量
    List* attrs;            //属性数组(每个属性是1个字符串)
    bool is_all_attrs;      //是否查询所有属性，当目标属性未指定时，默认查询所有属性
    
} ArrayScan;

typedef struct DocumentScan {
    Scan scan;
} DocumentScan;

typedef struct VectorScan {
    Scan scan;
} VectorScan;
...
typedef struct GraphScan {
    Scan scan;
    List* cypher_rels;
    List* cypher_restrictexprlist;          /* RestrictInfo structures (if graph rel) */
} GraphScan;
