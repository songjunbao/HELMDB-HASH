...
static Plan* create_array_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses);
static Plan* create_document_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses);
static Plan* create_vector_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses);
static Plan* create_graph_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses);
...
static Plan* create_scan_plan(PlannerInfo* root, Path* best_path)
{
    ...
    switch (best_path->pathtype) {
        ...
        case T_ForeignScan:
            ...
            break;
        case T_ArrayScan:
            plan = (Plan*)create_array_scan_plan(root, best_path, tlist, scan_clauses);
            break;
        case T_DocumentScan:
            plan = (Plan*)create_document_scan_plan(root, best_path, tlist, scan_clauses);
            break;
        case T_VectorScan:
            plan = (Plan*)create_vector_scan_plan(root, best_path, tlist, scan_clauses);
            break;
        case T_GraphScan:
            plan = (Plan*)create_graph_scan_plan(root, best_path, tlist, scan_clauses);
            break;
        default: {
            ereport(ERROR,
                (errmodule(MOD_OPT),
...
static bool use_physical_tlist(PlannerInfo* root, RelOptInfo* rel)
{
    ...
    ListCell* lc = NULL;
    /*
     * if it's a cypher query, then the graph table is only a virtual table
     * so that we can't use physical_tlist.
     */
    if(rel->mm_type == GRAPH_TABLE_MODEL_TYPE){
        return false;
    }
    /*
...
static SeqScan* create_seqscan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses)
{
    ...
    /* check the model type of table */
    scan_plan->mm_type = rte->mm_type;
    ...
    PlannerInfo* root, IndexPath* best_path, List* tlist, List* scan_clauses, bool indexonly, Bitmapset** out_prefixkeys)
{
    ...
    /* check the model type of table */
    rte = planner_rt_fetch(baserelid, root);
    scan_plan->mm_type = rte->mm_type;
    return scan_plan;
}
...
static SeqScan* make_seqscan(List* qptlist, List* qpqual, Index scanrelid)
{
    ...
    node->mm_type = NORMAL_TABLE_MODEL_TYPE;
    return node;
}
    ...
    List* indexqualorig, List* indexorderby, List* indexorderbyorig, ScanDirection indexscandir, double indexselectivity, bool is_partial)
{
    ...
    node->scan.mm_type = NORMAL_TABLE_MODEL_TYPE;
    return node;
}
...
// 注: 目前只通过浅拷贝的方式把scan结构体复制到arrayScan内部，可能会出现双重释放等问题,后期需考虑
static Plan* create_array_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses){
    ArrayScan* arrayScan = makeNode(ArrayScan);
    // 先构造seqscan plan，再把它放进arrayScan内部
    SeqScan *seqscan = create_seqscan_plan(root, best_path, tlist, scan_clauses);
    arrayScan->scan = *seqscan;

    // 把节点类型改成T_ArrayScan
    arrayScan->scan.plan.type = T_ArrayScan;

    // 加上数组扫描相关信息
    ArraySelectClause* original =(ArraySelectClause*)(root->parse->arraySelectClause);
    if (original == NULL){
        elog(NOTICE, "root->parse->arraySelectClause为空,执行普通关系表扫描...\n");
         return (Plan*)seqscan;
    }
    pfree(seqscan);

    // 复制基本和标量类型
    arrayScan->array_name = pstrdup(original->array_name);
    arrayScan->dims_num = original->dims_num;
    arrayScan->is_point_query = original->is_point_query;
    arrayScan->attrs_num = original->attrs_num;
    arrayScan->is_all_attrs = original->is_all_attrs;

    // 复制维度相关数组
    if (original->lower_bounds) {
        arrayScan->lower_bounds = (int*) palloc(original->dims_num * sizeof(int));
        memcpy(arrayScan->lower_bounds, original->lower_bounds, original->dims_num * sizeof(int));
    } else {
        arrayScan->lower_bounds = NULL;
    }

    if (original->upper_bounds) {
        arrayScan->upper_bounds = (int*) palloc(original->dims_num * sizeof(int));
        memcpy(arrayScan->upper_bounds, original->upper_bounds, original->dims_num * sizeof(int));
    } else {
        arrayScan->upper_bounds = NULL;
    }

    if (original->lower_bounds_valid != NULL) {
        arrayScan->lower_bounds_valid = (bool*) palloc(original->dims_num * sizeof(bool));
        memcpy(arrayScan->lower_bounds_valid, original->lower_bounds_valid, original->dims_num * sizeof(bool));
    } else {
        arrayScan->lower_bounds_valid = NULL;
    }

    if (original->upper_bounds_valid != NULL) {
        arrayScan->upper_bounds_valid = (bool*) palloc(original->dims_num * sizeof(bool));
        memcpy(arrayScan->upper_bounds_valid, original->upper_bounds_valid, original->dims_num * sizeof(bool));
    } else {
        arrayScan->upper_bounds_valid = NULL;
    }
    ...
    if (original->attrs) {
        arrayScan->attrs = NIL;
        ListCell *cell;
        foreach(cell, original->attrs) {
            char* data = (char*)lfirst(cell);
            arrayScan->attrs = lappend(arrayScan->attrs, strdup(data));
        }
    } else {
        arrayScan->attrs = NULL;
    }

    return (Plan*)arrayScan;
}

static Plan* create_document_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses){
    DocumentScan* documentScan = makeNode(DocumentScan);

    // create seqscan firstly
    SeqScan *seqscan = create_seqscan_plan(root, best_path, tlist, scan_clauses);

    // fill in the DocumentScan information
    documentScan->scan = *seqscan;
    documentScan->scan.plan.type = T_DocumentScan;

    pfree(seqscan);
    return (Plan*)documentScan;
}

static Plan* create_vector_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses){
    VectorScan* vectorScan = makeNode(VectorScan);

    // create seqscan firstly
    SeqScan *seqscan = create_seqscan_plan(root, best_path, tlist, scan_clauses);

    // fill in the VectorScan information
    vectorScan->scan = *seqscan;
    vectorScan->scan.plan.type = T_VectorScan;

    pfree(seqscan);
    return (Plan*)vectorScan;
}

static Plan* create_graph_scan_plan(PlannerInfo* root, Path* best_path, List* tlist, List* scan_clauses){
    GraphScan* graphScan = makeNode(GraphScan);

    // create seqscan firstly
    SeqScan* scan_plan = create_seqscan_plan(root, best_path, tlist, scan_clauses);
    GraphScanPath* path = (GraphScanPath*)best_path;
    
    // pass where info
    ListCell* lc = NULL;
    foreach(lc, path->cypher_rels){
        RelOptInfo* rel_opt_info = (RelOptInfo*)lfirst(lc);
        List* sub_scan_clauses = extract_actual_clauses(rel_opt_info->baserestrictinfo, false);
        scan_plan->plan.qual = list_concat(scan_plan->plan.qual, sub_scan_clauses);
    }

    // fill in the GraphScan information
    graphScan->scan = *scan_plan;
    graphScan->scan.plan.type = T_GraphScan;
    
    // pass MATCH info to GraphScan
    graphScan->cypher_rels = path->cypher_rels;
    graphScan->cypher_restrictexprlist = path->cypher_restrictexprlist;

    pfree(scan_plan);
    return (Plan*)graphScan;
}
