...
bool checkDMLRelation(const Relation rel, const PlannedStmt *plannedstmt, bool isInsert, bool isPartTbl)
{
    ...
    if ((rel->rd_rel->relkind != RELKIND_RELATION &&
         !(rel->rd_rel->relkind == RELKIND_FOREIGN_TABLE && isNVMFromTblOid(rel->rd_node.relNode))) ||
        rel->rd_rel->relhasrules || rel->rd_rel->relhastriggers || rel->rd_rel->relhasoids ||
        rel->rd_rel->relhassubclass || RelationIsColStore(rel) || RelationIsTsStore(rel) ||
...
FusionType CheckTargetListIsSimple(List *targetList)
{
    ListCell *lc = nullptr;
...
static inline bool isNvmPlan(Plan *top_plan)
{
    bool ret = false;
    if (IsA(top_plan, ForeignScan)) {
        ForeignScan *scan = (ForeignScan *)top_plan;
        if (isNVMFromTblOid(scan->scan_relid)) {
            ret = true;
        }
    }
    return ret;
...
FusionType getNvmSelectFusionType(Plan *top_plan)
{
    if (IsA(top_plan, LockRows)) {
        top_plan = top_plan->lefttree;
    }

    if (isNvmPlan(top_plan)) {
        ForeignScan *scan = (ForeignScan *)top_plan;
        FusionType type = CheckTargetListIsSimple(scan->scan.plan.targetlist);
        if (type != BYPASS_OK) {
            return type;
        }

        if (scan->sqlByPass) {
            return NVM_SELECT_FUSION;
        } else {
            return NOBYPASS_FOREIGN_SELECT_NOT_SUPPORT;
        }
    } else {
        return SELECT_FUSION;
    }
}
...
FusionType getSelectFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    ftype = getNvmSelectFusionType(top_plan);
    if (ftype != SELECT_FUSION) {
        return ftype;
    }
...
FusionType getInsertFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    if (rel->rd_rel->relkind == RELKIND_FOREIGN_TABLE && isNVMFromTblOid(relid)) {
        ftype = NVM_INSERT_FUSION;
    }
    heap_close(rel, NoLock);
    /*
     * check targetlist
...
FusionType getNvmUpdateFusionType(Plan *top_plan)
{
    ForeignScan *scan = (ForeignScan *)top_plan;
    if (scan->sqlByPass) {
        return NVM_UPDATE_FUSION;
    } else {
        return NOBYPASS_FOREIGN_UPDATE_NOT_SUPPORT;
    }
}
...
FusionType getUpdateFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    if (isNvmPlan(updatePlan)) {
        return getNvmUpdateFusionType(updatePlan);
    }
...
FusionType getNvmDeleteFusionType(PlannedStmt *plannedstmt, Plan *top_plan)
{
    FusionType type;
    ForeignScan *scan = (ForeignScan *)top_plan;
    if (scan->sqlByPass) {
        ...
        Oid relid = getrelid(res_rel_idx, plannedstmt->rtable);
        Relation rel = heap_open(relid, AccessShareLock);
        if (checkDMLRelation(rel, plannedstmt, false, false)) {
            type = NOBYPASS_DML_RELATION_NOT_SUPPORT;
        } else {
            type = NVM_DELETE_FUSION;
        }
        heap_close(rel, AccessShareLock);
        return type;
    } else {
        return NOBYPASS_FOREIGN_DELETE_NOT_SUPPORT;
    }
}
...
FusionType getDeleteFusionType(List *stmt_list, ParamListInfo params)
{
    ...
    if (isNvmPlan(deletePlan)) {
        return getNvmDeleteFusionType(plannedstmt, deletePlan);
    }
